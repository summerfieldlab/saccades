...
class NumAsMapsum(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.rnn = RNN(hidden_size, hidden_size, hidden_size)
...
def save_dataset(fname, noise_level, size):
    data = toy.generate_dataset(noise_level, size)
    data.to_pickle(fname)
    return data

def get_dataset(noise_level, size):
    fname = f'toysets/toy_dataset_nl-{noise_level}_{size}.pkl'
...
def get_model(model_type, small_weights, train_on, outer):
    ...
    if model_type == 'num_as_mapsum':
        model = NumAsMapsum(in_sz, hidden_size, output_size).to(device)
    elif model_type == 'rnn_classifier':
        model = RNNClassifier(in_sz, hidden_size, output_size).to(device)
    else:
...
def main():
    ...
    config.cross_entropy = False if model_type=='num_as_mapsum' else True
    ...
    test_results.to_pickle(f'detailed_test_results_{base_name}.pkl')
